...
module Jekyll
  ...
  class DataPage < Page
    ...
    def initialize(site, base, index_files, dir, data, name, template, extension, api)
      @site = site
      @base = base
    ...
    def generate(site)
      ...
      if data
        data.each do |data_spec|
          ...
          ghostData = JSON.load(response)['posts']
          
          if site.layouts.key? template

            ghostData.map do |post|
              if post.nil?
                records = site.data[level]
              else
               records = records.select { |r| r[data_spec['filter']] } if data_spec['filter']
               records = records.select { |record| eval(data_spec['filter_condition']) } if data_spec['filter_condition']   
               site.pages << DataPage.new(site, site.source, index_files_for_this_data, dir, post, name, template, extension, api)
